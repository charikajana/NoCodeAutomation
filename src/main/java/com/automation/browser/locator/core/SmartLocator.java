...
public class SmartLocator {
    ...
    public Locator findSmartElement(String name, String parsedType, Locator scope, String frameAnchor, boolean includeHidden) {
        ...
        // PRIORITIZE HEALED LOCATOR
        if (activePlan != null && activePlan.hasMetadata("healed_locator")) {
            logger.debug("SmartLocator: Using HEALED locator from plan metadata");
            return (Locator) activePlan.getMetadataValue("healed_locator");
        }
        ...
        if (cleanName.matches("(?i).*\\s+(link|button|icon|checkbox|check box|radio|radio button|element|field|input|dropdown|drop down|select|textarea|text area|slider|range|progress bar|progressbar)$")) {
            // Extract the type descriptor
            String[] parts = cleanName.split("\\s+");
            // Handle multi-word hints like "progress bar", "check box", "radio button"
            String typeHint;
            if (cleanName.toLowerCase().endsWith("progress bar")) {
                typeHint = "progressbar";
            } else if (cleanName.toLowerCase().endsWith("check box")) {
                typeHint = "checkbox";
            } else if (cleanName.toLowerCase().endsWith("radio button")) {
                typeHint = "radio";
            } else {
            ...
            cleanName = cleanName.replaceAll("(?i)\\s+(link|button|btn|input|field|tab|icon|radio|radio\\s+button|checkbox|check\\s+box|dropdown|drop\\s+down|select|box|menu|card|item|element|option|header|title|label|slider|range|text\\s+area|textarea|progress\\s+bar|progressbar)$", "").trim();
        }
    ...
    private Locator findInContext(String name, String parsedType, Frame frame, Locator scope, boolean includeHidden) {
        ...
        if (bestScore > 30 && bestElement != null) {
            // Store the matched candidate metadata for healing/learning
            if (activePlan != null) {
                activePlan.setMetadataValue("last_matched_candidate", bestElement);
                activePlan.setMetadataValue("last_matched_score", bestScore);
            }
    ...
    public void setActivePlan(ActionPlan plan) {
        this.activePlan = plan;
    }

    public ActionPlan getActivePlan() {
        return activePlan;
    }

    /**
     * Attempts to heal a failing locator using historical fingerprints.
     */
    public Locator heal(ActionPlan plan) {
        String stepText = plan.getTarget();
        ElementFingerprint fingerprint = HealingRegistry.getInstance().get(stepText);
        
        if (fingerprint == null) {
            logger.debug("Healing Engine: No historical fingerprint found for step '{}'", stepText);
            return null;
        }

        logger.warning("Healing Engine: Attempting to heal step '{}' using fingerprint...", stepText);
        
        // Strategy 1: Try the exact XPath
        if (fingerprint.xpath != null && !fingerprint.xpath.isEmpty()) {
            try {
                logger.debug("Healing Strategy 1: Trying historical XPath '{}'", fingerprint.xpath);
                Locator healed = page.locator("xpath=" + fingerprint.xpath).first();
                if (healed != null && healed.isVisible()) {
                    logger.success("Healing Engine: SUCCESS! Re-located element via historical XPath.");
                    return healed;
                }
            } catch (Exception e) {
                logger.debug("XPath healing failed: {}", e.getMessage());
            }
        }

        // Strategy 2: Try the historical ID
        if (fingerprint.id != null && !fingerprint.id.isEmpty()) {
            try {
                logger.debug("Healing Strategy 2: Trying historical ID '{}'", fingerprint.id);
                Locator healed = page.locator("#" + fingerprint.id).first();
                if (healed != null && healed.isVisible()) {
                    logger.success("Healing Engine: SUCCESS! Re-located element via historical ID.");
                    return healed;
                }
            } catch (Exception e) {
                logger.debug("ID healing failed: {}", e.getMessage());
            }
        }

        logger.failure("Healing Engine: All healing strategies failed for step '{}'", stepText);
        return null;
    }
